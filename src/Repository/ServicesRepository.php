<?php

namespace App\Repository;

use App\Entity\Services;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Common\Persistence\ManagerRegistry;

/**
 * @method Services|null find($id, $lockMode = null, $lockVersion = null)
 * @method Services|null findOneBy(array $criteria, array $orderBy = null)
 * @method Services[]    findAll()
 * @method Services[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 */
class ServicesRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, Services::class);
    }

    // /**
    //  * @return Services[] Returns an array of Services objects
    //  */
    /*
    public function findByExampleField($value)
    {
        return $this->createQueryBuilder('s')
            ->andWhere('s.exampleField = :val')
            ->setParameter('val', $value)
            ->orderBy('s.id', 'ASC')
            ->setMaxResults(10)
            ->getQuery()
            ->getResult()
        ;
    }
    */

    /*
    public function findOneBySomeField($value): ?Services
    {
        return $this->createQueryBuilder('s')
            ->andWhere('s.exampleField = :val')
            ->setParameter('val', $value)
            ->getQuery()
            ->getOneOrNullResult()
        ;
    }
    */
    public function findUsers()
    {
        return $this->findOneBy([
            'service'      => 'users'
        ]);
    }

    public function findHome()
    {
        return $this->findOneBy([
            'service'      => 'accueil'
        ]);
    }

    public function findZones()
    {
        return $this->findOneBy([
            'service'      => 'zones'
        ]);
    }

    public function findPays()
    {
        return $this->findOneBy([
            'service'      => 'pays'
        ]);
    }

    public function findContinents()
    {
        return $this->findOneBy([
            'service'      => 'continents'
        ]);
    }

    public function findPaysVisaClassic()
    {
        return $this->findOneBy([
            'service'      => 'pays_visaclassic'
        ]);
    }

    public function findInfosEntrepriseVisaClassic()
    {
        return $this->findOneBy([
            'service'      => 'infosentreprises_visaclassic'
        ]);
    }

    public function findTypeVisaClassic()
    {
        return $this->findOneBy([
            'service'     => 'type_visaclassic'
        ]);
    }

    public function findCategorieVisaClassic()
    {
        return $this->findOneBy([
            'service'     => 'categorie_visaclassic'
        ]);
    }

    public function findVoletInfoVisaClassic()
    {
        return $this->findOneBy([
            'service'     => 'volets_infos_visaclassic'
        ]);
    }

    public function findRubriqueVisaClassic()
    {
        return $this->findOneBy([
            'service'     => 'rubriqueVisaClassic'
        ]);
    }
    

    

    

}
